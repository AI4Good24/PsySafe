...
class GroupChat:
    ...
    critic_in_loop: bool = True
    frequent_human_input: bool = True

    _VALID_SPEAKER_SELECTION_METHODS = ["auto", "manual", "random", "round_robin", "order"]
...
Then select the next role from {[agent.name for agent in agents]} to play. Only return the role."""
    ...
    def orderly_select_speaker(self, agents: List[Agent], agent_index) -> Agent:
        """orderly select the next speaker."""
    ...
    def select_speaker(self, last_speaker: Agent, selector: ConversableAgent, agent_index):
        ...
        if self.speaker_selection_method.lower() == "manual":
            selected_agent = self.manual_select_speaker(agents, agent_index)
            if selected_agent:
                return selected_agent
            
        if self.speaker_selection_method.lower() == "order":
            selected_agent = self.orderly_select_speaker(agents, agent_index)
            if selected_agent:
                return selected_agent
...
class GroupChatManager(UniversalAgent):
    ...
    def run_chat(
        ...
        if groupchat.critic_in_loop:   
            agent_list = [_i for _i in range(1, len(groupchat.agents))] # except for the user_proxy

        else:
            agent_list = [_i for _i in range(1, len(groupchat.agents)-1)] # except for the user_proxy and critic


        # [1 2] --> [1 0 2 0]
        if groupchat.frequent_human_input:
            _agent_list = []
            for i in range(len(agent_list)):
                _agent_list.append(agent_list[i])
                _agent_list.append(0)
            agent_list = _agent_list
        
            
            # inject human input between every two agent
        ...
        for i in range(groupchat.max_round):
            ...
            try:
                # select the next speaker
                speaker = groupchat.select_speaker(speaker, self, agent_list[agent_index%len(agent_list)])
                agent_index += 1
